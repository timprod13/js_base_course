...
const map = {
    ...
    render(snakePointsArray, foodPoint, obstaclePoint) {
		...
		const obstacleCell = this.cells[`x${obstaclePoint.x}_y${obstaclePoint.y}`]; //генерируем препятствие как еду
...
const game = {
    ...
    reset() {
		...
		this.obstacle.setCoordinates(this.getRandomFreeCoordinates(this.obstacle));
        this.render();
    },
    ...
    tickHandler() {
        ...
        if (!this.canSnakeMakeStep() || this.obstacle.isOnPoint(this.snake.getNextStepHeadPoint())) this.finish(); // добавили условие для препятствия
	...
	increasePoints () { // считаем очки в реальном времени
        this.points += 1;
        const score = document.getElementById('score-value');
        score.innerText = 'Score: ' + this.points;
    },

    displayPoints () { // заглушка для обнуления количества очков по нажатию кнопки Новая игра
        const score = document.getElementById('score-value');
		this.points = 0;
